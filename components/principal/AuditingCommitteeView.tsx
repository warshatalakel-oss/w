...
import AuditingCommitteePDFPage from './AuditingCommitteePDFPage.tsx';

declare const jspdf: any;
declare const html2canvas: any;

interface AuditingCommitteeViewProps {
...
const defaultTasks = `١- تدقيق سجلات الدرجات والتأكد من صحة الجمع والنقل.
٢- مطابقة الدرجات بين سجل المدرس والسجل العام.
٣- التأكد من تطبيق قرارات وزارة التربية المتعلقة بالدرجات (مثل درجات القرار).
٤- تدقيق النتائج النهائية للطلاب (ناجح, مكمل, راسب).
٥- رفع تقرير نهائي الى مدير المدرسة بصحة النتائج.`;

export default function AuditingCommitteeView({ setCurrentPageKey, settings, users }: AuditingCommitteeViewProps) {
    const [members, setMembers] = useLocalStorage<CommitteeMember[]>('examLog_auditingMembers', [
        { id: uuidv4(), name: settings.principalName, role: 'رئيس اللجنة' },
        { id: uuidv4(), name: '', role: 'عضو' },
        { id: uuidv4(), name: '', role: 'عضو' },
    ]);
    const [tasks, setTasks] = useLocalStorage('examLog_auditingTasks', defaultTasks);
    ...
    return (
        <PageWrapper 
            title="لجنة التدقيق وأعمالها" 
            onPrev={() => setCurrentPageKey('committee')}
            onNext={() => setCurrentPageKey('specialization_committees')}
        >
            <div className="bg-gray-50 p-6 rounded-lg border shadow-sm space-y-6">
                <div>
                    <h3 className="font-bold text-lg mb-2">أعضاء لجنة التدقيق:</h3>
